...
A menu-bar-only macOS 14+ utility (version **0.0.11**) that shows remaining **Cursor**, **ChatGPT**, **GLM** (z.ai / BigModel coding plan), and **Grok** (consumer SuperGrok) subscription quota. One dark pill in the status bar, one compact popover. No Dock icon, no telemetry.
...
xcodebuild -project QuotaBar.xcodeproj -scheme QuotaBar -configuration Release \
  MARKETING_VERSION=0.0.11 CURRENT_PROJECT_VERSION=0.0.11 \
...
That writes `dist/QuotaBar.app` and `dist/QuotaBar.zip` at version 0.0.11, then runs `codesign --force --deep --sign -`.
