...
A menu-bar-only macOS 14+ utility (version **0.0.15**) that shows remaining **Cursor**, **ChatGPT**, **GLM** (z.ai / BigModel coding plan), **Grok** (consumer SuperGrok), and **OpenCode Go** subscription quota. One dark pill in the status bar, one compact popover. No Dock icon, no telemetry.
...
xcodebuild -project QuotaBar.xcodeproj -scheme QuotaBar -configuration Release \
  MARKETING_VERSION=0.0.15 CURRENT_PROJECT_VERSION=0.0.15 \
...
That writes `dist/QuotaBar.app` and `dist/QuotaBar.zip` at version 0.0.15, then runs `codesign --force --deep --sign -`.
