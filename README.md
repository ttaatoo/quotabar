...
- Popover (not a detached window): fixed chrome (`312×300`) so switching Cursor / ChatGPT / GLM does not move the arrow. Provider title + plan badge, signed-in email when the API provides one, relative “Updated …” time, refresh, a three-way provider switcher. The ChatGPT tab lists every account at once (email + plan + that account’s classified meters). Empty Session windows are omitted. Settings… and Quit QuotaBar stay in the footer.
- Settings: enable each provider, add / rename / delete ChatGPT accounts (Add account runs `codex login` in the default browser), paste Cursor / GLM credentials in compact secret fields, GLM region, poll interval (default 120s), remaining vs used, launch at login (`SMAppService`), and an off-by-default **Preview fixtures** toggle for screenshots.
...
4. Signed-in email is shown under the Cursor header (and on the Settings card) when Cursor publishes one: `usage-summary` if it includes an email field, otherwise `GET https://cursor.com/api/auth/me` (`email`), otherwise the session JWT `email` claim. If none of those have an address, QuotaBar shows nothing rather than a fake email.
...
10. In the popover, pick **ChatGPT**. Every account is listed together as a compact card (email, plan badge, that account’s Weekly / Session meters). There is no account pill/tab strip. The menu-bar percentage is the most-constrained remaining % among **visible** ChatGPT accounts (orange below 25%).
