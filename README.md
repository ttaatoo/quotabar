...
A menu-bar-only macOS 14+ utility (version **0.0.10**) that shows remaining **Cursor**, **ChatGPT**, **GLM** (z.ai / BigModel coding plan), and **Grok** (consumer SuperGrok) subscription quota. One dark pill in the status bar, one compact popover. No Dock icon, no telemetry.
...
- Status item: dark rounded pill with a small bar-chart mark and a percentage (remaining quota for the selected provider’s most-constrained window, usually weekly). When ChatGPT is selected, that percentage is the remaining % of the **active** ChatGPT account only (click a card in the popover to choose; the choice is saved in `~/.config/quotabar/config.json`). The number turns orange below 25% remaining.
...
xcodebuild -project QuotaBar.xcodeproj -scheme QuotaBar -configuration Release \
  MARKETING_VERSION=0.0.10 CURRENT_PROJECT_VERSION=0.0.10 \
...
That writes `dist/QuotaBar.app` and `dist/QuotaBar.zip` at version 0.0.10, then runs `codesign --force --deep --sign -`.
...
10. In the popover, pick **ChatGPT**. Every account is listed together as the same compact card used by Cursor / GLM / Grok (email, plan badge, that account’s Weekly / Session meters, Credits footer inside the card). Click a card to make it the **active** account (thin purple ring and a small check). There is no account pill/tab strip and the email is not in the header. Extra accounts scroll inside the body. The menu-bar percentage is that active account’s remaining % only (the Weekly number the card already shows; orange below 25%). A single account is active automatically. If the active account is deleted or missing after refresh, QuotaBar falls back to the first remaining signed-in account, or the unsigned-in empty state if none remain.
